...
pdf(paste0(fig_path, "grfs.pdf"), width = 6, height = 2.4) #open pdf
op <- par(mfrow=c(1,3)) #initiate subplotting
contour(field1, xlab="x", ylab="y") #,  plot.title = {par(cex.main=1);title(main = "Realization 1")})
contour(field2, xlab="x", ylab="y") #, plot.title = {par(cex.main=1);title(main = "Realization 2")})
contour(field3, xlab="x", ylab="y") #, plot.title = {par(cex.main=1);title(main = "Realization 3")})
...
# pdf(paste0(fig_path,"empiricalvariogram36points.pdf"))#, width = 3, height = 2)
...
ymax = max(emp_variogram9$v, emp_variogram36$v, emp_variogram64$v, emp_variogram100$v)


#pdf(paste0(fig_path,"empiricalvariograms9-36-64-100.pdf"), width=6, height=4) #open pdf
#op <- par(mfrow=c(2,2), mar = c(2, 2, 0, 2) + 0.5) #start subplotting
pdf(paste0(fig_path,"empiricalvariograms9.pdf"), width=4, height=3)
par(mgp=c(2,1,0), mar=c(0.5,1,-0.5,-1)+2.5) #mgp changes distance between axes 
                                            #and labels, mar changes margins
plot(emp_variogram9, type="l", xlab=TeX("$\\tau$"), ylab=TeX("$\\gamma_r$"), ylim=c(0,ymax))
...
ggsave(paste0(fig_path,"parameterestimation.pdf"))
